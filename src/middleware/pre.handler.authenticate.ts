...
import {
  E400_MISSING_PAYLOAD,
  E401_MISSING_ACCESS_TOKEN,
  E401_UNAUTHORIZED_ACCESS,
  E403_ACCESS_TOKEN_FORBIDDEN
...
const pre_handler_authenticate: RouteShorthandOptions['preHandler'] = async function(
  ...
  if (token === MISSING_ACCESS_TOKEN) {
    return reply.code(401).send(E401_MISSING_ACCESS_TOKEN)
  }

  request.jwtVerify(async (err, cUsr) => {
    if (err) {
      log(err)
      reply.code(403).send(E403_ACCESS_TOKEN_FORBIDDEN)
      return
    }
    ...
    if (isBlacklisted) {
      ...
      reply.code(401).send(E401_UNAUTHORIZED_ACCESS)
      return
    }
    ...
    if (!cUsr) {
      ...
      reply.code(400).send(E400_MISSING_PAYLOAD)
      return
    }
